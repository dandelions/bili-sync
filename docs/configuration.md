...
开启后，程序只请求并下载 DASH 音频流，不请求视频流，并将音频封装为 `.m4a` 文件。视频封面、NFO、弹幕和字幕等其它处理仍按原有开关执行。

该选项既可在全局“视频处理”设置中开启，也可在单个视频源的“覆盖全局流设置”中单独开启。若某个视频只提供混合视频流，或没有可用的独立音频流，该分页会失败并记录错误，不会退化为下载视频。
