...
视频名称模板（`video_name`）和分页名称模板（`page_name`）保留用于兼容已有配置，但下载文件现在统一直接保存到合集、投稿、收藏等视频源配置的根目录下，不再按视频名称或 `Season 1` 创建子目录。文件名使用视频标题；多分页视频会追加 ` - P01`、` - P02` 等分页编号。对于所有下载的内容，目录结构如下：
...
开启后，程序只保存 `.m4a` 音频文件。若视频只提供混合流，则会下载完整混合流，再由 `ffmpeg` 提取或转换音频；这种情况下仍会产生视频流量和临时文件。音频文件使用与视频相同的名称模板和目录。

### 同时保存视频和音频

开启后，程序同时保存视频和音频：视频文件保存在原视频目录，音频文件保存在该视频目录下的 `Audio` 子目录中，并使用相同的文件名和 `.m4a` 扩展名。此选项会下载视频流和音频流，增加磁盘占用。

这两个选项既可在全局“视频处理”设置中开启，也可在单个视频源的“覆盖全局流设置”中单独开启。若视频没有可用音频轨道，或 `ffmpeg` 无法解析该流，该分页会失败并记录错误。
