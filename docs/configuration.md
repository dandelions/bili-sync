...
开启后，程序优先只请求并下载 DASH 音频流，并将音频封装为 `.m4a` 文件。若视频只提供混合流，则会下载完整混合流，再由 `ffmpeg` 提取或转换音频；这种情况下仍会产生视频流量和临时文件。视频封面、NFO、弹幕和字幕等其它处理仍按原有开关执行。

该选项既可在全局“视频处理”设置中开启，也可在单个视频源的“覆盖全局流设置”中单独开启。若视频没有可用音频轨道，或 `ffmpeg` 无法解析该流，该分页会失败并记录错误。
